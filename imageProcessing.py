...
def calculatepHashPhotos(assets, immich_server_url, api_key):
    if 'message' not in st.session_state or st.button('开始处理'):
    ...
    for i, asset in enumerate(assets):
        ...
        if not isAssetProcessed(asset_id):
            ...
            if image is not None:
                ...
                processed_assets += 1
                st.session_state['message'] += f"已处理并保存资产 {asset_id}\n"
                
                # Explicitly delete the image object and free memory
                del image
                gc.collect()
            else:
                st.session_state['message'] += f"获取资产 {asset_id} 的图像失败\n"
                error_assets += 1
        else:
            st.session_state['message'] += f"资产 {asset_id} 已处理，跳过。\n"
            skipped_assets += 1
        ...
        st.session_state['message'] += f"预计剩余时间: {estimated_time_remaining_min} 分钟\n"
        st.session_state['message'] += f"资产 {i + 1} / {total_assets} - (已处理 {processed_assets} - 已跳过 {skipped_assets} - 错误 {error_assets})\n"
